...
namespace FlatFiles.Scrutinizer
{
    ...
    public sealed class FixedLengthReader : IReader
    {
        ...
        private string[] partitionRecord(string record)
        {
            ...
            if (record.Length > schema.TotalWidth || record.Length < schema.TotalWidth)
            {
                throw new LengthProcessingException(SharedResources.InvalidLengthError, record.Length, schema.TotalWidth);
            }
        ...
        public object[] GetRawValues()
        {
            ...
            object[] copy = new object[rawValues.Length];
            Array.Copy(rawValues, copy, rawValues.Length);
            return copy;
        }
    }
}
