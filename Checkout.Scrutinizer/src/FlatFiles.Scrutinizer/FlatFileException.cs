...
namespace FlatFiles.Scrutinizer
{
    ...
    public sealed class ColumnProcessingException : FlatFileException
    {
        ...
        private static string getErrorMessage(ISchema schema, IColumnDefinition definition, string value)
        {
            ...
            string message = String.Format(null, SharedResources.InvalidColumnConversion, value, definition.ColumnType.FullName, definition.ColumnName);
    ...
    public sealed class LengthProcessingException : FlatFileException
    {
        internal LengthProcessingException(string message, int expectedLength, int actualLength)
            : base(String.Format(null, message, expectedLength, actualLength))
        {
            this.ExpectedLength = expectedLength;
            this.ActualLength = actualLength;
        }

        internal LengthProcessingException(string message, int expectedLength, int actualLength, Exception innerException)
            : base(String.Format(null, message, expectedLength, actualLength), innerException)
        {
            this.ExpectedLength = expectedLength;
            this.ActualLength = actualLength;
        }
